...
from data_generator import (
    ...
    get_or_generate_transfer_predictions,
    get_or_generate_shap_importance,
)
...
@st.cache_data
def load_data():
    # Tries the real CSV first; falls back to on-the-fly generation
    return get_or_generate_dataset('transferiq_final_dataset.csv', n_players=500)

@st.cache_data
def load_transfer_predictions(_df):
    # Underscore prefix tells Streamlit not to hash the dataframe argument
    return get_or_generate_transfer_predictions(
        _df, path='players_with_transfer_predictions.csv'
    )
...
df_transfer      = load_transfer_predictions(df)
models           = load_models()
...
# ============================================================================
    ...
    # df_transfer is always populated via load_transfer_predictions(),
    # but guard defensively just in case.
    if df_transfer is None:
        df_transfer = get_or_generate_transfer_predictions(df)
...
# ============================================================================
    ...
    # Always succeeds: loads real CSV if present, otherwise generates one
    shap_importance = get_or_generate_shap_importance(
        'shap_feature_importance_market_value.csv'
    )
    ...
    if player_explain:
        ...
        # Derive player-specific contributions from real feature values
        top8_features = top_features['feature'].head(8).tolist()
        contribs = []
        for feat in top8_features:
            if feat in df.columns:
                raw      = float(pd_e[feat])
                col_mean = float(df[feat].mean())
                col_std  = float(df[feat].std()) or 1.0
                contrib  = round((raw - col_mean) / col_std * 3.5, 2)
            else:
                contrib = 0.0
            contribs.append({'Feature': feat, 'Contribution': contrib})

        mock = pd.DataFrame(contribs).sort_values('Contribution', ascending=True)
        ...
        st.caption(
            "ℹ️ Contributions show how this player’s stats deviate from the "
            "dataset average. Positive = above average (raises value); "
            "Negative = below average (lowers value)."
        )
